...
- **`notifications`** — `MailTemplate`, `Invito` (token UUID + `tipo`), `TipoInvito`, `render_mail()`; service + tasks Celery. Namespace `notifications`. Template default in `templates/mail/`.
  ...
  **`GestioneInvitiView`** (`GET /notifiche/inviti/`): dashboard con contatori (da invitare / inviato / attivato) e tabella per squadriglia. **`InviaInvitiEdizoneView`** (`POST /notifiche/inviti/invia/`): avvia il task bulk (`tipo=capi|csq`).
