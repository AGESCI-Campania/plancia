...
- Social login Google/Microsoft/Apple (guida: `docs/guide/social_auth.md`).
  **Configurazione Apple Sign In** (`SOCIALACCOUNT_PROVIDERS['apple']['APP']`): `key`=TEAM_ID (usato come `iss` nel JWT), `secret`=KEY_ID (usato come `kid`), `settings.certificate_key`=chiave privata PEM. Attenzione: la versione precedente aveva i campi scambiati causando 401 al callback.
...
- **`accounts`** — `User` (+ `socio` OneToOneField), `Nomina`, `LoginEvent`; `roles.py` (+ service `nomina()`); `signals.py` (+ `ensure_superuser_ruolo`: imposta automaticamente `ruolo=ADMIN` quando `is_superuser=True` viene salvato — corregge il default CSQ di `createsuperuser`); `mixins.py`; `forms.py` (Bootstrap mixin + form allauth); `views.py` (`ProfiloView`, `UtenteListView`, `UtenteDetailView`, `NominaView`, `CambiaRuoloView`, `CreaUtenteDaSocioView`); `urls.py` (namespace `accounts`, mountato su `utenti/`). Template: `accounts/profilo|utente_list|utente_detail`. Test: `apps/accounts/tests/`.
  **Account social nel profilo**: `ProfiloView.get_context_data` popola `social_accounts` (`SocialAccount.objects.filter(user=u)`). Il template `profilo.html` mostra una card "Account social collegati" con provider, email e pulsante "Scollega" (POST a `socialaccount_connections` con `account=<pk>`).
...
- **`editions`** — `Edizione` (FSM 4 stati), `Dilazione`; CRUD views + template. Namespace `editions`.
  `EdizioneDetailView`: tabella diari con ricerca per nome CRP/CSQ, filtri Zona/Gruppo/Specialità e ordinamento colonne (client-side JS). `_diari_visibili` usa `select_related("squadriglia__reparto__gruppo__zona", "csq", "crp", "anagrafica")`; il context include `zone`, `gruppi`, `specialita_list` per i dropdown.
...
- **`notifications`** — `MailTemplate`, `Invito` (token UUID + `tipo` + delivery tracking), `TipoInvito`, `DeliveryStatus`, `render_mail()`; service + tasks Celery + webhooks anymail. Namespace `notifications`. Template default in `templates/mail/`.
  ...
  **`GestioneInvitiView`** (`GET /notifiche/inviti/`): dashboard con contatori (da invitare / inviato / attivato) e tabella per squadriglia con badge delivery, ricerca per nome CRP/CSQ, filtri Zona/Gruppo, ordinamento colonne (client-side JS). **Contatori CRP deduplicati**: `_calcola_stato_inviti` aggrega gli inviti per CRP univoco (PK Socio) tramite le prefetch — un CRP con N squadriglie conta come 1. **`InviaInvitiEdizoneView`** (`POST /notifiche/inviti/invia/`): avvia il task bulk (`tipo=capi|csq`). **`InvitiCrpView`** (`GET /notifiche/inviti/miei/`): accessibile al solo Capo Reparto — elenca le proprie squadriglie con stato dell'invito al Capo Squadriglia e pulsante "Reinvia" (chiama `ReinvioInvitoView` che ora ammette anche `Ruolo.CRP` per i propri inviti).
  **Attivazione CSQ**: `AttivazoneInvitoView` — per `TipoInvito.CODICE_SOCIO` mostra il form `notifications/conferma_codice_socio.html` (GET), valida codice socio + email (POST), aggiorna `User.email`/`Socio.email` se placeholder o diversa, poi autentica. `_attiva_e_login` crea automaticamente una `Nomina` per il `ruolo_target` se mancante (necessario per utenti già esistenti con ruolo diverso, es. Admin che è anche Capo Reparto); aggiorna `User.ruolo` solo se era il primo ruolo attivo.
...
- **`siteconfig`** — `Impostazioni` singleton, middleware manutenzione, backend email custom; `forms.py` (`ImpostazioniForm` con widget Bootstrap, `MailTemplateForm` con TinyMCE). Namespace `siteconfig`.
  ...
  **Dual email backend**: `BackendPosta` choices (`smtp`/`transazionale`). `Impostazioni.email_backend_standard` (default: smtp) per email di sistema e inviti singoli; `email_backend_massivo` (default: transazionale) per inviti bulk CRP/CSQ. `email_mode` sovrascrive entrambi (MAILPIT/SIMULATO). `get_connection_per_tipo(tipo, fail_silently)` in `email_backends.py` è la funzione di routing usata dal service. `PlanciaEmailBackend` (EMAIL_BACKEND globale) delega a `get_connection_per_tipo('standard')`. `_TeeBackend` usato per SIMULATO_PIU_INVIO (scrive su file E invia).
  **Gmail OAuth2 SMTP**: `GmailSMTPCredenziali` model (account_email, access_token, refresh_token, expires_at). `GmailOAuth2Backend(SmtpBackend)` usa XOAUTH2 su smtp.gmail.com:587. Flusso OAuth in `/impostazioni/gmail-smtp/oauth/` (stesso client ID del Drive, scope `https://mail.google.com/`, PKCE). `_get_gmail_access_token(creds)` rinnova il token via `google-auth`. Env: `GOOGLE_GMAIL_SMTP_REDIRECT_URI`. Views: `GmailSMTPOAuthInitView`, `GmailSMTPOAuthCallbackView`, `GmailSMTPOAuthDisconnectView`. Migrazione siteconfig 0008.
  **Routing inviti bulk**: `invia_inviti_capi/csq_per_edizione(edizione, backend_tipo='massivo')` e `task_invia_inviti_*_edizione(edizione_pk, backend_tipo='massivo')` accettano override. `GestioneInvitiView` espone `backend_massivo_label` nel contesto; i form di invio bulk hanno un selettore backend.
  **Email provider**: `EmailProvider` choices (smtp / brevo / mailgun / mailersend / postmark / sendgrid / sparkpost / ses). Nuovi campi: `email_provider`, `email_provider_api_key`, `email_provider_webhook_secret`.
  ...
  **Invito tracking**: `DeliveryStatus` choices (in_attesa/inviato/consegnato/bounce/spam/fallito); campi `provider_message_id`, `delivery_status`, `delivery_error` su `Invito`. Visualizzati in `notifications/_delivery_badge.html` (incluso in `gestione_inviti.html`).
  Pagina impostazioni suddivisa in sezioni (Identità, Footer, Posta elettronica, Stato, Import, Pagine legali, Template email).
  **Pagine statiche** (`PaginaStatica`): modello con `slug` (`SlugPagina.PRIVACY`/`TERMINI`), `titolo`, `contenuto` (TinyMCE). URL pubblici `/privacy/` e `/termini/` (no login). Edit: `PaginaStaticaEditView` (`GET/POST /impostazioni/pagine/<slug>/`), solo Admin/Segreteria/IABR. Se il record non esiste viene creato al primo accesso in edit. Il footer mostra i link "Privacy Policy" e "Condizioni del Servizio" sopra il `footer_testo` su tutte le pagine. Cookie banner GDPR in `base.html`: banner fisso in basso (cookie tecnici essenziali), dismissibile via `localStorage`, con link a `/privacy/`.
...
**Pagine di errore**: `templates/404.html` e `templates/403.html` estendono `base.html` con `--ag-primary: #dc3545` (navbar rossa). `templates/500.html` è standalone (Django non passa context alle 500) con Bootstrap via CDN e navbar rossa minimale.
...
- **Email**: dual backend — `email_backend_standard` (smtp) per sistema, `email_backend_massivo` (transazionale) per inviti bulk. Gmail SMTP via OAuth2 XOAUTH2 (`GmailSMTPCredenziali`). `get_connection_per_tipo(tipo)` è la funzione di routing in `email_backends.py`; `email_mode` sovrascrive tutto (SIMULATO/MAILPIT/REALE). Guida: `docs/guide/email_provider.md`.
