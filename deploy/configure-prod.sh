...
# Genera .env.prod, sceglie la modalita' di proxying e la porta, e rende il vhost.
...
# ----------------------------------------------------------------------------
render() { sed -e "s|\${SERVER_NAME}|${SERVER_NAME}|g" -e "s|\${APP_PORT}|${APP_PORT}|g" "$1"; }
case "$PROXY_MODE" in
  nginx-host)
    OUT="deploy/plancia.nginx.conf"; render deploy/nginx.vhost.tpl > "$OUT"
    echo "vhost nginx generato: $OUT"
    echo "  -> copialo in /etc/nginx/sites-available/, abilitalo e: nginx -t && systemctl reload nginx";;
  apache-host)
    OUT="deploy/plancia.apache.conf"; render deploy/apache.vhost.tpl > "$OUT"
...
if [[ "$RUN_COMPOSE" == "yes" ]]; then
  ...
  echo "Configurazione completata. Per avviare:"
  echo "  COMPOSE_PROFILES=${COMPOSE_PROFILES} docker compose --env-file .env.prod up -d"
fi
